...
export const getUser = async (email) => {
    ...
    // la url 
    const url = `https://obscure-wildwood-55661.herokuapp.com/users?email=${email}`;
...
export const login = async (email, password) => {
    ...
    const url = "https://obscure-wildwood-55661.herokuapp.com/tokens";
    const payload = { 
...
export const signup = async (email, password, userName)=>{
    const answer = {}
    //urls for signup
    const url = "https://obscure-wildwood-55661.herokuapp.com/users";
    const payloadSignup = { 
          email: email.toLowerCase(),
          password: password,
          userName: userName
...
export const logout= async (email) => {
  //urls for log out
  const url = `https://obscure-wildwood-55661.herokuapp.com/tokens?email=${email}`;
  const token = await AsyncStorage.getItem('@cp_token')
  // fetch con el metodo y el headers
  const res = await fetch(url, {
